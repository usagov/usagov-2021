...
| H-07 | Open | | |
| H-08 | Open | | |
| H-09 | Partial | `cadf16df0` | Missing requested components now fail closed. Date-based pairing in `find_corresponding_backup` unchanged |
| H-10 | Open | | |
| H-11 | Open | | |
| H-12 | Open | | |
...
| N-11 | Open | `printf '%q'` is a Bash extension, not POSIX, and appears at five sites in `#!/bin/sh` scripts (`deploy.sh` ×2, `local-manager.sh` ×3). Under `dash` it emits `printf: %q: invalid directive` and the arguments are lost: `exec_backup_command` then runs `manager.sh backup` with no types, ticket or suffix, producing a wrongly-tagged backup. Verified by running the downsync suite under `dash` — 26 assertions fail, identically before and after the H-03 work, so this is pre-existing and not a regression. macOS and the containers are unaffected because their `/bin/sh` is Bash and BusyBox respectively. Folds into H-22, which already tracks the declared-shell mismatch |
...
A hermetic suite for the downsync backs this up: 78 assertions over 16 scenarios,
passing under `bash` and `/bin/sh`, using fake `cf`/`aws`/`drush` and two fake spaces.
...
Still open, and the substance of this finding: `find_corresponding_backup` still pairs components by extracting the first `YYYY-MM-DD` and taking the first same-day candidate, ignoring ticket, environment, container, suffix, sequence, and `LastModified`. A restore can still combine components from different backup events on the same day. Fixing that needs the backup-set manifest from the target architecture, not a patch here.
