    ...
    foreach ($xpath->query('/html/head/script[contains(@id, "taxonomy-data")]') as $node) {
      ...
      if (!empty($csv)) {
        ...
        foreach ($csv as $key => $line) {
          ...
          if (!empty($line[$nodeIDElement]) && !empty($decoded["nodeID"]) && $line[$nodeIDElement] == $decoded["nodeID"]) {
            if ($line[$languageElement] == $content_replace[$decoded["language"]]) {
              $pointer = $key;
            }
          }

          if (!empty($line[$nodeIDElement]) && !empty($decoded["taxonomyID"]) && $line[$nodeIDElement] == 't_' . $decoded["taxonomyID"]) {
            if ($line[$languageElement] == $content_replace[$decoded["language"]]) {
              $pointer = $key;
            }
          }
