...
tomestatic="mkdir -p /tmp/betahtml && drush cr --root=${www} && drush tome:static -y --uri=$URI --process-count=10 --path-count=10 --root=${www} && sleep 60 && cp -rf $theme/fonts $html/themes/custom/usagov/ && cp -rf $theme/images $html/themes/custom/usagov/ && cp -rf ${www}/html /tmp/betahtml"
