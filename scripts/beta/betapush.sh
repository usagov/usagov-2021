...
tomestatic="mkdir -p /tmp/betahtml && drush cr --root=${www} && drush tome:static -y --uri=$URI --process-count=10 --path-count=10 --root=${www} && sleep 60 && cp -rf ${www}/html /tmp/betahtml && echo 'done copying file from webroot(html) to tmp(html)'"
