...
if [ -f "$SITEMAP_FILE" ]; then
  ...
  # xhtml:link rel attribute
  REL_TEXT=' rel="alternate"'

  # Add a "/" to the end of "localhost/es" in the sitemap and remove the promo comment, the stylesheet and `rel="alternate"` from xhtml:link.
  sed -i -e "s|/es\"/>|/es/\"/>|g" -e "s|/es</loc>|/es/</loc>|g" -e "s|$STYLING_TEXT||g" -e "s|$PROMO_TEXT||g" -e "s|$REL_TEXT||g" $SITEMAP_FILE;
