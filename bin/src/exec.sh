...
docker exec -it -u nginx cms sh -c 'drush -y config-set system.performance css.preprocess 0'
docker exec -it -u nginx cms sh -c 'drush -y config-set system.performance js.preprocess 0'
